...
import type { User, Transaction, KYCStatus, KycRequest, PendingDeposit, Notification, ChatMessage, PendingWithdrawal, SecondContractTrade, UserDetails, KYCData, LoginRecord, SystemSettings, ReferralInfo, VipTier } from '../types.ts';
...
const initialDb: MockDB = {
  ...
  systemSettings: {
    ...
    homepageActionItems: [
      { id: 'recharge', order: 1, enabled: true, label: 'Recharge', icon: 'CreditCard', path: '/profile', state: { view: 'deposit' } },
      { id: 'withdraw', order: 2, enabled: true, label: 'Withdraw', icon: 'Download', path: '/profile', state: { view: 'withdraw' } },
      { id: 'balance', order: 3, enabled: true, label: 'Balance', icon: 'Scale', path: '/profile', state: {} },
      { id: 'support', order: 4, enabled: true, label: 'Support', icon: 'Headset', path: 'https://t.me/FortressInvestmentSupport', state: {} },
    ],
    vipTiers: [
...
const updateUserVipAndDeposits = (user: User, vipTiers: VipTier[]) => {
    ...
    const sortedTiers = [...(vipTiers || [])].sort((a, b) => b.depositThreshold - a.depositThreshold);
...
const ensureCompleteUser = (user: Partial<User>, vipTiers: VipTier[]): User => {
    ...
    updateUserVipAndDeposits(completeUser, vipTiers);
...
const loadDB = (): MockDB => {
    ...
    try {
        ...
        if (rawDb && rawDb !== 'undefined') {
            ...
            // Ensure admin user is always present and has the correct password from initialDb
...
const findUserByEmail = (email: string): User | undefined => {
  ...
  return db.users[lowerCaseEmail];
};
...
export const apiSignup = (name: string, email: string, password: string, details: UserDetails, referralCode?: string): Promise<{user: User, token: string}> => {
  ...
  const partialNewUser: Partial<User> = { 
      ...
      portfolio: { balance: 100, pendingBalance: 0, pl: 0, plPercentage: 0, referralRewards: 0, totalDeposits: 0 },
      transactions: [{
        id: `bonus-${uid}`,
        type: 'Signup Bonus',
        asset: 'USDT',
...
export const apiGetAiChatResponse = async (token: string, message: string): Promise<User> => {
    const uid = token.replace('TOKEN_', '');
    const user = findUserByUid(uid);
    if (!user) return apiError("User not found.");
    ...
    if (!process.env.API_KEY) {
        user.chatHistory.push({ role: 'model', parts: [{ text: "Hello! This is a mock response as the AI service is not connected. I can help with market concepts. I cannot provide financial advice." }] });
        saveDB();
        const { password: _, ...userToReturn } = user;
        return apiCall(userToReturn, 1500);
    }
    
    try {
        const ai = new GoogleGenAI({apiKey: process.env.API_KEY});
        const chat: Chat = ai.chats.create({ model: 'gemini-2.5-flash', history: user.chatHistory.slice(0, -1),
            config: { systemInstruction: "You are Fortress Investment Assistant. You are an expert in cryptocurrency and financial markets. Explain complex topics simply. Always state that you cannot give financial advice. Keep answers concise." }
        });
        const result: GenerateContentResponse = await chat.sendMessage({message});
        user.chatHistory.push({ role: 'model', parts: [{ text: result.text }] });
    } catch (error) {
        user.chatHistory.push({ role: 'model', parts: [{ text: "I'm sorry, I encountered an error. Please try again later." }] });
    } finally {
        saveDB();
        const { password: _, ...userToReturn } = user;
        return apiCall(userToReturn, 100);
...
export const apiUpdateDepositStatus = (token: string, adminPasswordAttempt: string, targetUserId: string, transactionId: string, status: 'Completed' | 'Failed'): Promise<void> => {
    ...
    if (status === 'Completed') {
        ...
        updateUserVipAndDeposits(targetUser, db.systemSettings.vipTiers || []); // Update VIP Level
